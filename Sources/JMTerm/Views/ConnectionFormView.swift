...
struct ConnectionFormView: View {
    ...
    /// 연결 실패 원인을 단계별 친화 메시지로 분류(ConnectionErrorText 위임).
    private func diagnose(_ error: Error) -> String {
        ConnectionErrorText.diagnose(error, port: port)
    }
    ...
    private func testConnection() {
        ...
        Task {
            do {
                ...
                testState = .failure(message: diagnose(error), detail: ConnectionErrorText.describe(error))
            }
        }
    }
}
